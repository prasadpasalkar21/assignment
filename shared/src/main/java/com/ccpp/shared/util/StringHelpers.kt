...
object StringHelpers {
    ...
    fun getRefNo(): String {
        ...
        val timeStamp: String = dateFormat.format(calender.time).replace("-", "")
    ...
    fun diffTwoDate(time:Date?): Boolean
    {
        val currentTime = getDateFormat(Calendar.getInstance().time.toString())
        val diff = currentTime!!.time - time!!.time

        val days =  (diff / (1000*60*60*24)).toInt()
        val hours =  ((diff - (1000*60*60*24*days)) / (1000*60*60)).toInt()
        val min =  ((diff - (1000*60*60*24*days) - (1000*60*60*hours)) / (1000*60)).toInt()

        println(min.toString() +"< 2" )
    ...
    {
        val sdf3: SimpleDateFormat = SimpleDateFormat("EEE MMM dd HH:mm:ss zzz yyyy", Locale.ENGLISH)
        try {
    ...
    fun gmtToIst(date:String?):String?
    {
        val utcFormat: DateFormat = SimpleDateFormat("EEE MMM dd HH:mm:ss zzz yyyy", Locale.ENGLISH)
        utcFormat.timeZone = TimeZone.getTimeZone("UTC")
        val indianFormat: DateFormat =
            SimpleDateFormat("EEE dd MMMM yyyy hh:mm:ss a", Locale.ENGLISH)
        indianFormat.timeZone = TimeZone.getTimeZone("Asia/Kolkata")
        val timestamp: Date = utcFormat.parse(date)
